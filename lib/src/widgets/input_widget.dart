...
class _InputWidgetState extends State<InternationalPhoneNumberInput> {
  ...
  @override
  Widget build(BuildContext context) => _InputWidgetView(
        state: this,
      );
  ...
  Future<void> initialiseWidget() async {
    if (widget.initialValue != null) {
      if (widget.initialValue!.phoneNumber != null &&
        ...
        final String phoneNumber = await PhoneNumber.getParsableNumber(widget.initialValue!);
  ...
  void phoneNumberControllerListener() {
    if (mounted) {
      final String parsedPhoneNumberString = controller!.text.replaceAll(RegExp(r'[^\d+]'), '');

      getParsedPhoneNumber(parsedPhoneNumberString, country?.alpha2Code).then((phoneNumber) {
        if (phoneNumber == null) {
          final String phoneNumber = '${country?.dialCode}$parsedPhoneNumberString';
  ...
  String? validator(String? value) {
    final bool isValid = isNotValid && (value!.isNotEmpty || widget.ignoreBlank == false);
...
class _InputWidgetView extends WidgetView<InternationalPhoneNumberInput, _InputWidgetState> {
  const _InputWidgetView({required this.state, super.key}) : super(state: state);
  @override
